...
type MQTTConsumer struct {
	...
	TopicMapperOn          bool              `toml:"topic_mapper_on"`
	TopicColumnMaps        []string          `toml:"topic_column_maps"`
...
func (m *MQTTConsumer) mapTopic(metric telegraf.Metric, topic string) {
	topicCols := strings.Split(topic, "/")
	topicMap := m.findMatchingMap(len(topicCols))

	if len(topicCols) != len(topicMap) {
		return
	}

	for i, topicCol := range topicCols {
		mapCol := topicMap[i]

		if mapCol == "name" {
			metric.SetName(topicCol)
		} else if strings.HasPrefix(mapCol, "tag") {
			mapColTagKey := strings.Split(mapCol, ":")
			if len(mapColTagKey) != 2 {
				m.Log.Debugf("A topic map tag should have a 'tag:tagKey' format")
				return
			}
			metric.AddTag(mapColTagKey[1], topicCol)
		} else if strings.HasPrefix(mapCol, "field") {
			mapColFieldKey := strings.Split(mapCol, ":")
			if len(mapColFieldKey) != 2 {
				m.Log.Debugf("A topic map field should have a 'field:fieldKey' format")
				return
			}
			existingVal, ok := metric.GetField(mapColFieldKey[1])
			if ok {
				metric.AddField(topicCol, existingVal)
				metric.RemoveField(mapColFieldKey[1])
			}
		}
	}
}

func (m *MQTTConsumer) findMatchingMap(len int) []string {
	for _, topicMap := range m.TopicColumnMaps {
		if strings.Count(topicMap, "/")+1 == len {
			return strings.Split(topicMap, "/")
		}
	}
	return []string{}
}
